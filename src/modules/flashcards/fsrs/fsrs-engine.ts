...
export class FSRSEngine {
 ...
 // Main function to process a card review
  processReview(card: Card, rating: Rating): Partial<Card> {
    console.log('FSRS processReview called:', {
      cardId: card.id,
  ...
  sortByPriority(cards: Card[]): Card[] {
    ...
    return cards.sort((a, b) => {
      ...
      if (a.nextReview && b.nextReview) {
        const aDays = (now.getTime() - a.nextReview.getTime()) / (1000 * 60 * 60 * 24)
        const bDays = (now.getTime() - b.nextReview.getTime()) / (1000 * 60 * 60 * 24)
