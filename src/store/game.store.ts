...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  // Start new game
  startGame: (gameType) => {
    logger.debug('Starting new game', { gameType })
    ...
    logger.info('Game started successfully', { gameType, sessionId: session.startedAt })
  },
  
  // End game
  endGame: () => {
    const { currentSession } = get()
    logger.debug('Ending game', { currentSession })
    if (currentSession) {
      // TODO: Save session to database
      logger.info('Game session ended', { 
        gameType: currentSession.gameType,
        score: currentSession.score,
        linesCleared: currentSession.linesCleared,
  ...
  hideFlashcard: () => {
    set({ 
      flashcardVisible: false,
      currentFlashcard: null,
      nextFlashcardThreshold: Math.floor(Math.random() * 9) + 3 // 3-11 - combined update
    })
  },
