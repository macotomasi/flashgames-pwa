...
export default function TetrisGameComponent() {
  ...
  useEffect(() => {
    ...
    game.spawnPiece = async function() {
      ...
      logger.debug('Piece progress', { current: pieceCountRef.current, threshold: nextThresholdRef.current })
  ...
  const loadCards = async (forFun: boolean = false) => {
    ...
    if (cards.length === 0 && !forFun) {
      ...
      logger.debug('Cards loaded for review', { cardsCount: cards.length, forFun })
    }
  }
  ...
  useEffect(() => {
    if (isNative) return // Skip keyboard controls on mobile
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-900 text-white flex flex-col items-center justify-center p-2 md:p-4 overflow-auto">
      ...
      <div className="flex gap-8">
        ...
        {isNative && (
          <div className="fixed bottom-2 left-1/2 transform -translate-x-1/2 flex flex-col gap-2 z-10">
